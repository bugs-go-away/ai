...
export const createChat = async (req, res, next) => {
	...
	if (opponentId === undefined || !username) {
		return next({
			...
			status: 400,
			message:
				'server says: you have to specify an opponentId and username',
		});
	}
	...
	try {
		...
		if (message.deletedCount == 0) {
			return next({
				...
				status: 400,
				message:
					'Server says: this username does not have an active session, you cant end this chat!',
			});
		}
		...
		return next({
			...
			status: 500,
			message:
				'error ending chat session, check your username and password.',
		});
	}
};
	...
	try {
		...
		if (message === null) {
			return next({
				...
				status: 400,
				message:
					'Server says: this username does not have an active session, call init to create one!',
			});
		}
		...
		return next({
			...
			status: 400,
			message:
				'error getting chat session data, check your username and password.',
		});
	}
};
...
export const saveConversationToChat = async (_req, res, next) => {
	...
	try {
		let message = await Chat.findOneAndUpdate(
